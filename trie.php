...
:root {
  --main: #bd0034;    /* Main theme color */
  --light: #ffffff;   /* Light color, used for text or backgrounds */
  --dark: #000000;    /* Dark color, typically used for text */
  --bg: #ff0046;      /* Background color for highlighted elements */
}
...
.gjs-sm-sector .gjs-sm-title {
 ...
 .gjs-cv-canvas { /* targeting the canvas */
        min-height: 100%;
        
    }
    .gjs-block { /* targeting toolbox elements */
        flex-basis: auto;
    }
...
.modal-content { background-color: #fefefe; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 80%; }
  ...
  <!-- Container for the editor -->
    <div class="buttons">
        <button id="save-btn" onclick="save()">Save Project</button>
        <input type="file" id="load-input" />
        <button id="save-template">Save Template</button>
        <button id="load-btn">Load Project</button>
        <button id="" onclick="change()">toggle side bar</button>
      <button id="post-data">Post/Publish</button>  
    </div>
    ...
    <script>
      ...
      var editor = grapesjs.init({
        ...
        assetManager: {
            ...
            upload: './handler.php', // Server-side script tbo handle uploads
            uploadName: 'files',
            multiUpload: true,
            assets:[
  {
    "type": "image",
    "src": "./posts/images/colorful-design-vector-4603131.jpg",
    "name": "colorful-design-vector-4603131.jpg"
  },
  {
    "type": "image",
    "src": "./posts/images/1 (9)-min (1).jpg",
    "name": "1 (9)-min (1).jpg"
  },
  {
    "type": "image",
    "src": "./posts/images/pexels-andre-furtado-43594-1264210.jpg",
    "name": "pexels-andre-furtado-43594-1264210.jpg"
  },
  {
    "type": "image",
    "src": "./posts/images/food-min.jpg",
    "name": "food-min.jpg"
  },
  {
    "type": "image",
    "src": "./posts/images/1 (9)-min.jpg",
    "name": "1 (9)-min.jpg"
  }
],
...
editor.Commands.add('open-images-panel', {
    run: function (editor, sender) {
        sender.set('active', !sender.get('active'));
        // Toggle visibility of a hypothetical image panel or perform actions to show images
        showMyImagePanel(sender.get('active'));
    },
});

function showMyImagePanel(show) {
    // Logic to show/hide your custom panel with images
}
...
// Add button to the panel
        ...
        function openPostModal() {
            ...
            const modalContent = `
                ...
                <input type="file" id="template-image" />

                <button onclick="postData()">Save Template</button>
    ...
    const updatedHtml = htmlContent.replace('./edits/images/', '../images/')
...
editor.Commands.add('set-canvas-size', {
    run: function(editor, sender) {
        ...
        if (canvasWidth) {
            editor.Canvas.setDimensions({ width: canvasWidth });
        }
    }
});
...
editor.on('load', () => {
        ...
        const defaultStyle = cssComposer.addRule('body', { width: '100vw' });
    });
    editor.on('load', () => {
    editor.setStyle(`
        body {
...
// Call the function on editor load or whenever appropriate
        ...
        document.getElementById('post-data').addEventListener('click', function() {
            ...
            fetch('./php_script.php', {
                ...
                body: JSON.stringify({html: updatedHtml, css: cssContent})
            })
            .then(response => response.json())
            .then(data => {
